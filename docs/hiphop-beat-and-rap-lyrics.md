...
```mermaid
graph TD
    A[BPM 분석] -->|BPM 115 기준 분류| B{장르 판별}
    B -->|BPM < 115| C[붐뱁: 7~14음절]
    B -->|BPM >= 115| D[트랩: 14~24음절]
...
* **장르별 분류 임계값**: **BPM 115** (단, 하프타임 비트 고려)
  * **하프타임 보정**: 분석된 BPM이 **60 ~ 80 BPM 범위**에 속할 경우, 이를 120 ~ 160 BPM of 하프타임 트랩 비트로 간주하여 **BPM 값을 2배 곱해준 뒤** 분류합니다.
  * **BPM 115 미만** ➔ **장르: 붐뱁** (목표 음절 수: **7 ~ 14음절**)
  * **BPM 115 이상** ➔ **장르: 트랩** (목표 음절 수: **14 ~ 24음절**)
...
clean_line = re.sub(r"^\(\d+음절\)\s*", "", clean_line) # 앞단 "(X음절)" 가이드 제거
clean_line = re.sub(r"\(\d+음절\)\s*$", "", clean_line) # 뒷단 "(X음절)" 가이드 제거 (하위 호환성)
```
