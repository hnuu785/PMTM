...
check("숫자만 입력 시 한글 음독 음소 반환", len(get_phonemes("12345")) == 9, f"got {get_phonemes('12345')}")
...
# 끝 음절 글자가 완벽히 똑같지는 않으나 1음절 문장의 경우 음절 전체 '강' vs '방'
check("'강'↔'방' = 1.0", approx(s, 1.0), f"got {s}")

s = get_line_rhyme_score("강산", "방판")
# 끝 2음절 모음 일치하며, 끝 음절 자음(초성)이 다르고 단어가 다르므로 1.0
...
# 바(ㅏ,_)다(ㅏ,_) vs 마(ㅏ,_)차(ㅏ,_) -> 모든 끝 음절 모음 일치, 끝 초성 다름 -> 1.0
