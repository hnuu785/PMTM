...
class GrpoMessagesTests(unittest.TestCase):
    ...
    def test_build_prompts_outputs_chat_messages(self):
        ...
        msg1 = prompts[0]
        self.assertEqual([m["role"] for m in msg1], ["system", "user"])
        self.assertIn("랩을 작성해주세요", msg1[1]["content"])
        self.assertIn("10~14 범위 내로", msg1[1]["content"])
        self.assertNotIn("AAAABBBB 스키마를 준수", msg1[1]["content"])

        # 2nd prompt (bpm=140, 트랩)
        msg2 = prompts[1]
        self.assertEqual([m["role"] for m in msg2], ["system", "user"])
        self.assertIn("랩을 작성해주세요", msg2[1]["content"])
        self.assertIn("14~18 범위 내로", msg2[1]["content"])
        self.assertNotIn("AAAABBBB 스키마를 준수", msg2[1]["content"])
