...
- 모델이 BPM 숫자를 크기 비교하여 장르를 판별하는 수학적 추론 오차가 날 수 있으므로, 처음부터 `장르: 트랩`, `목표 음절: 14~24` 형태로 주입하여 오차를 방지했다.
- **BPM 115**을 임계값 기준으로 붐뱁과 트랩 모두 `6~18음절`로 맵핑한다.
...
CLI 인자 `--genre`, `--mood`는 `generate_for_api.py`에서 완전히 제거되었다.
