...
def determine_genre_from_bpm(bpm: float) -> str:
    """MVP용 BPM 기반 장르 규칙."""
    return "boom_bap" if bpm < 105 else "trap"


def build_snare_map(beat_analysis: dict[str, Any]) -> dict[int, dict[str, float]]:
    """신뢰도 기준을 통과한 snare event를 절대 grid slot 기준으로 변환한다."""
    events = beat_analysis.get("snare_detection", {}).get("events", [])
    ...
    for event in events:
        ...
        if not isinstance(grid_slot, int):
            continue
        if not isinstance(confidence, (int, float)) or confidence < SNARE_CONFIDENCE_MIN:
            continue

        candidate = {
            "confidence": float(confidence),
            "original_time": float(event.get("original_time", 0.0)),
            "snapped_time": float(event.get("snapped_time", 0.0)),
        }
        current = snare_map.get(grid_slot)
        if current is None or candidate["confidence"] > current["confidence"]:
            snare_map[grid_slot] = candidate
    return snare_map
...
def calculate_micro_offset(
    ...
    """강세와 snare anchor에 따라 micro offset을 계산한다."""
    rules = MICRO_OFFSET_RULES[genre]
    absolute_slot = int(syllable["start_absolute_slot"])
    snare_info = snare_map.get(absolute_slot)
    stress = float(syllable.get("stress", 0.0))

    if snare_info is not None:
    ...
    offset_ms = max(-MICRO_OFFSET_MAX_MS, min(MICRO_OFFSET_MAX_MS, offset_ms))
    return {"offset_ms": round(offset_ms, 3), "reason": reason, "snare_info": snare_info}


def apply_micro_offsets(
    syllables: list[dict[str, Any]],
    genre: str,
    slot_duration: float,
    snare_map: dict[int, dict[str, float]],
) -> None:
    """원본 start/end는 보존하고 micro timing 필드를 추가한다."""
    for syllable in syllables:
        result = calculate_micro_offset(syllable, genre, slot_duration, snare_map)
        offset_ms = float(result["offset_ms"])
        offset_sec = offset_ms / 1000.0
        syllable["micro_offset_ms"] = offset_ms
        syllable["micro_offset_reason"] = result["reason"]
        syllable["micro_start_sec"] = round(float(syllable["start_sec"]) + offset_sec, 6)
        syllable["micro_end_sec"] = round(float(syllable["end_sec"]) + offset_sec, 6)
        syllable["micro_duration_sec"] = round(
            syllable["micro_end_sec"] - syllable["micro_start_sec"], 6
        )

        snare_info = result["snare_info"]
        syllable["snare_reference"] = None if snare_info is None else {
            "grid_slot": int(syllable["start_absolute_slot"]),
            "confidence": round(snare_info["confidence"], 4),
            "original_time": round(snare_info["original_time"], 6),
            "snapped_time": round(snare_info["snapped_time"], 6),
        }
...
def make_segment(
    ...
    apply_micro_offsets(syllables, genre, slot_duration, snare_map)
...
def create_flow_plan(
    ...
    bpm = float(beat_analysis["bpm"]["fixed_integer"])
    genre = determine_genre_from_bpm(bpm)
    snare_map = build_snare_map(beat_analysis)
    lines, warnings = [], []
    ...
    for line_index, original_text in enumerate(lyrics, start=1):
        ...
        for segment_index, (segment_text, layout) in enumerate(zip(lyric_segments, layouts), start=1):
            ...
            segment = make_segment(
                ...
                genre=genre, snare_map=snare_map,
            )
            if segment["density"] == "overflow":
                warnings.append({
    ...
    flow_plan = {
        "metadata": {
            ...
            "bpm": bpm,
            "genre_rule": genre,
            "micro_offset_enabled": True,
            "micro_offset_max_ms": MICRO_OFFSET_MAX_MS,
            "snare_confidence_min": SNARE_CONFIDENCE_MIN,
            "snare_event_count": len(snare_map),
            "downbeat_offset_sec": beat_analysis["downbeat_offset_sec"],
