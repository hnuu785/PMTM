...
가사 LLM은 최종 플로우 타이밍을 결정하지 않는다. 다만 비트에서 파생된 FlowBrief를 조건으로 받아, 의미·라임 품질을 유지하면서 발화 가능하고 플로우를 설계하기 쉬운 가사를 생성한다. 정확한 구절 위치, 음소 duration, 쉼, 강세와 F0는 Flow/Timing Planner가 결정한다.

플로우는 가사의 고정 속성이 아니라 가사를 비트 안에서 어떻게 발화할지에 대한 시간적 해석이다. 같은 가사와 같은 비트에도 half-time, double-time, 당김음, 쉼, 강세와 구절 분할이 다른 여러 유효한 플로우가 존재한다. 따라서 PMTM은 비트마다 하나의 정답 플로우나 정답 음절 수가 있다고 가정하지 않는다. FlowBrief는 이 여러 가능성을 닫는 정답 타이밍이 아니라, 목표 마디 수, tempo 해석, 구간별 밀도, 필수 쉼, 강세 앵커와 허용 subdivision을 요약한 고수준 생성 조건이다.
...
| FlowBrief Builder | 비트 분석 결과와 사용자 선택 | 목표 마디 수, tempo 해석, 구간별 밀도, 필수 쉼, 강세 앵커와 허용 subdivision 요약 | 고수준 FlowBrief |
| Lyrics LLM | 주제, 가사 스타일 조건과 FlowBrief | 주제 전개, 자연스러운 한국어, 라임, 반복 억제와 발화 가능성 | 편집 가능한 8줄 가사 |
...
2. 분석 결과와 사용자 선택으로 고수준 FlowBrief를 만든다.
3. 사용자가 입력한 주제와 FlowBrief에 맞춰 랩 가사를 생성한다.
4. 사용자가 생성된 가사를 직접 수정한다.
5. 별도의 Flow/Timing Planner가 최종 가사와 비트를 조합해 하나 이상의 정밀 플로우를 만든다.
6. 선택한 플로우로 가이드 보컬을 합성하고 비트와 병합한다.
...
플로우 밀도, tempo 해석, 필수 쉼과 강세 앵커의 고수준 의도는 FlowBrief를 통해 가사 LLM에 전달한다. 정확한 박자 위치, 음소 duration, 쉼 길이, 강세 세기와 F0는 Flow/Audio 단계의 선택값이다. 초기 제품에서는 비트 분석 결과로 FlowBrief 기본값을 제안하고 사용자가 바꿀 수 있게 한다.

### 4.2 랩 가사 생성

가사 LLM은 FlowBrief를 고려하되 정확한 비트 타이밍을 예측하지 않고 다음 품질에 집중한다.

- 사용자가 제시한 주제와 의미 전개
- 자연스러운 한국어
- 끝 라임과 내부 라임
- 동일 줄, 끝단어와 구절의 반복 억제
- FlowBrief의 목표 밀도와 쉼을 수용할 수 있는 발화 가능성
- 편집하기 쉬운 정확한 8줄 형식

초기 DiffSinger 검증에서는 8줄을 정확히 8마디로 해석해 `1줄 = 1마디`로 고정한다. 이후 일반화 단계에서만 한 줄을 여러 구절로 나누거나 여러 줄을 이어 발화하는 배치를 검토한다.
...
이 정보의 원시 분석값과 정밀 타임스탬프를 가사 LLM의 정답으로 직접 사용하지 않는다. 대신 분석 결과를 고수준 FlowBrief로 요약해 가사 생성 조건으로 제공하고, 전체 분석값은 Flow/Timing Planner와 오디오 정렬에 사용한다. 박자표나 마디 경계를 자동으로 확정하기 어려우면 4/4를 기본값으로 제안하되 사용자가 수정할 수 있게 한다.
...
- 원하는 분위기(선택)
- 라임 스킴과 라임 강도(선택)
- 비트에서 파생된 FlowBrief: 목표 마디 수, tempo 해석, 구간별 밀도, 필수 쉼, 강세 앵커와 허용 subdivision

원시 BPM, 박자표와 비트 타임스탬프를 그대로 정답처럼 주입하거나 목표 음절 수, 박자 위치, 음소 duration, 강세 세기와 F0를 가사 LLM이 확정하게 하지 않는다. 가사 LLM은 FlowBrief의 고수준 조건만 사용하고, 정밀 타이밍은 Flow/Timing Planner가 결정한다.
...
가사 LLM 학습에서 단순 줄 길이나 BPM별 고정 음절 범위를 정답 플로우로 최적화하지 않는다. 대신 검증된 FlowBrief 조건 준수와 발화 가능성을 평가하고, 정확한 타이밍·프로소디 품질은 Flow/Audio 파이프라인의 데이터와 청취 평가로 별도 개선한다.
...
현재 8마디 SVS 검증 구현은 하나의 결정론적 후보만 생성한다. 각 가사 줄을 한 마디에 배치하고 음절 밀도에 따라 8분음표, 16분음표 또는 32분음표 기반 템플릿을 선택하며, 사용자는 첫 마디 시작 위치만 보정한다.
...
**완료 기준:** 8줄 형식, 주제 관련성, 한국어 자연스러움, 라임, 반복 억제와 FlowBrief 기반 발화 가능성 기준을 통과한다. 정밀 타이밍이나 합성 보컬 품질은 가사 모델 완료 기준으로 사용하지 않는다.
...
- 사람 평가 기반 한국어 자연스러움
- FlowBrief 조건 준수율과 발화 가능성 사전 점수
...
가사 LLM의 모델 승격은 의미·언어·라임 지표와 FlowBrief 기반 발화 가능성 사전 지표로 결정한다. 정밀 플로우와 비트 위 합성 보컬 품질은 Flow/Audio 버전의 승격 기준으로 별도 관리하고, 최종 제품 릴리스에서는 두 계층을 조합한 통합 청취 평가를 추가한다.

## 11. 범위와 원칙

- PMTM의 1차 목적은 상업 발매용 완성 음원을 자동 제작하는 것이 아니라, 작사와 플로우를 빠르게 검토할 수 있는 가이드 데모를 제공하는 것이다.
- 같은 가사와 비트에 하나의 정답 플로우가 있다고 가정하지 않는다.
- 가사 LLM에는 비트에서 파생된 고수준 FlowBrief를 제공하되, BPM별 고정 음절 수나 줄 길이, 음소 타이밍으로 하나의 플로우를 강제하지 않는다.
